...
def prepare_train_and_test_data(configs, args):
    ...
    if not os.path.exists(train_one_path) \
        ...
        first_count = prepare_karaoke_tfrecord(
            train_one_path, configs, 0, 45, 0)
        second_count = prepare_karaoke_tfrecord(
            train_two_path, configs, 45, 60, 0, second_set=True)
...
def prepare_val_data(configs):
    ...
    samples, _ = librosa.load(
        configs['TEST_FILE'], sr=configs['SAMPLE_RATE'], mono=True)
    ...
    mix_frames = sample_frames(
        true_mix_stft, configs['STACKED_FRAMES'], hop_sample)
    mix_frames = np.reshape(
        mix_frames, (len(mix_frames), configs['WINDOW_SIZE']))
...
def prepare_karaoke_tfrecord(set_path, configs, set_start_int, set_stop_int, dataset_break, second_set=False):
    ...
    for track_index in range(set_start_int, set_stop_int-1):
        ...
        if not start_dataset_two:
            ...
            for vocal_stem in mix['target_stems']:
                ...
                samples, _ = librosa.load(
                    stem, sr=configs['SAMPLE_RATE'], mono=True)
            ...
            for stem in track_files:
                ...
                print(stem_path)
                samples, _ = librosa.load(
                    stem_path, sr=configs['SAMPLE_RATE'], mono=True)
                if stem == 'vocals.wav':
                    vocal_stems.append(samples)
                else:
                    nonvocal_stems.append(samples)
        ...
        vstem = vocal_stems
        vocal_stems = None
        nstem = nonvocal_stems
        nonvocal_stems = None
        vocal_stems = np.array(vstem, dtype=np.float64)
        vstem = None
        nonvocal_stems = np.array(nstem, dtype=np.float64)
        nstem = None
        vocal_mix = mix_stems(vocal_stems)
        nonvocal_mix = mix_stems(nonvocal_stems)
        true_mix = np.add(np.multiply(vocal_mix, 0.5),
                          np.multiply(nonvocal_mix, 0.5))
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            true_masks = np.true_divide(
                vocal_mag, np.add(vocal_mag, nonvocal_mag))
        ...
        mask_frames = sample_frames(
            true_masks, configs['STACKED_FRAMES'], hop_sample)
        mix_frames = sample_frames(
            true_mix_stft, configs['STACKED_FRAMES'], hop_sample)

        mask_frames = np.reshape(
            mask_frames, (len(mask_frames), configs['WINDOW_SIZE']))
        mix_frames = np.reshape(
            mix_frames, (len(mix_frames), configs['WINDOW_SIZE']))
...
def stft(mix, configs):
    ...
    _, _, Zxx = signal.stft(
        mix, window='hann', nperseg=fft_size, noverlap=amount_of_overlap, nfft=fft_size)
    return Zxx
...
def istft(mix, configs):
    fft_size = configs['FFT_SIZE']
    amount_of_overlap = fft_size - configs['HOP_SIZE']
    _, x = signal.istft(mix, window='hann', nperseg=fft_size,
                        noverlap=amount_of_overlap, nfft=fft_size)
    return x
